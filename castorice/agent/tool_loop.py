...
class ToolLoopMixin:
    ...
    def _step_tool_loop(self, state, stream_callback=None):
        ...
        messages = [ChatMessage("system", system_prompt, cacheable=True)]
        ...
        for round_num in range(MAX_TOOL_ROUNDS):
            try:
                ...
                t0 = time.time()
                response = self._call_llm_with_tools(messages, tool_schemas, use_native_fc)
                latency_ms = (time.time() - t0) * 1000

                self._record_llm_call_metrics(response, latency_ms)

                if not response.tool_calls:
                    self._handle_direct_answer(response.content, state, stream_callback)
                    return

                normalized_calls = self._prepare_tool_calls(response.tool_calls, round_num)
                result_msgs = self._execute_tool_calls(normalized_calls, state, response.content, use_native_fc)
                self._process_tool_results(messages, normalized_calls, result_msgs, response.content, use_native_fc)
        ...
        if not state.final_answer and not state.errors:
            self._handle_loop_timeout(state)

    def _call_llm_with_tools(self, messages: List[ChatMessage], tool_schemas: List[Dict[str, Any]],
                             use_native_fc: bool) -> Any:
        """调用 LLM 获取响应（支持原生 FC 和 JSON 解析两种模式）"""
        if use_native_fc:
        ...
        if not tool_calls:
            ...
            if decision.get("action") == "answer":
                response.content = decision.get("answer", content)
                response.tool_calls = []
                return response
            tc_name = decision.get("tool", "")
            tc_args = decision.get("args", {})
            if tc_name:
                response.tool_calls = [{"id": f"json_{id(content)}", "name": tc_name, "arguments": tc_args}]
            else:
                response.tool_calls = []
        else:
    ...
    def _record_llm_call_metrics(self, response: Any, latency_ms: float) -> None:
        """记录 LLM 调用指标和日志"""
        content = getattr(response, 'content', '')
        tool_calls = getattr(response, 'tool_calls', [])

        logger.info(
            f"工具循环 LLM响应: {content[:200] if content else '(tool_calls)'}"
            f" | tool_calls数={len(tool_calls) if tool_calls else 0}"
        )
    ...
    def _handle_direct_answer(self, content: str, state, stream_callback=None) -> None:
        """处理 LLM 直接给出最终答案的情况"""
    ...
    def _prepare_tool_calls(self, tool_calls: Any, round_num: int) -> List[Dict[str, Any]]:
        """
        解析并归一化 LLM 返回的工具调用，统一处理 dict 和 ToolCall 对象。
        """
        normalized_calls = []
    ...
    def _validate_tool_calls(self, tool_calls: List[Dict[str, Any]], state,
                             use_native_fc: bool) -> List[Optional[ChatMessage]]:
        """
        验证工具调用：检查工具是否存在、情绪拒绝、模式检测、授权检查。
        返回错误消息列表，None 表示通过验证。
        """
        results = []
        for tc in tool_calls:
            tool_name = tc["name"]
            ...
            if tool_name not in self.tools:
                results.append(self._handle_tool_not_found(tool_name, tc_id, use_native_fc))
                continue
            ...
            try:
                ...
                if refuse:
                    ...
                    if use_native_fc:
                        ...
                        results.append(ChatMessage("user", refuse_reason + " 请直接用你自己的话回答用户。"))
                    continue
            except Exception as e:
                logger.warning(f"L3 情绪拒绝检查失败: {e}")

            if self._is_pattern_detection_enabled():
                blocked = self._check_pattern_detection(tool_name, tool_args, tc_id, use_native_fc, state)
                if blocked is not None:
                    results.append(blocked)
                    continue

            if self._is_authorization_enabled():
                blocked = self._check_authorization(tool_name, tc_id, use_native_fc)
                if blocked is not None:
                    results.append(blocked)
                    continue
    ...
    def _execute_tool_calls(self, tool_calls: List[Dict[str, Any]], state, content: str,
                            use_native_fc: bool) -> List[Optional[ChatMessage]]:
        """
        执行工具调用，支持单工具同步执行和多工具并行执行。
        """
        if len(tool_calls) == 1:
            return [self._execute_single_tool_call(
                tool_calls[0], state, content, use_native_fc
            )]

        return self._execute_tool_calls_parallel(
            tool_calls, state, content, use_native_fc
        )

    def _process_tool_results(self, messages: List[ChatMessage], tool_calls: List[Dict[str, Any]],
                              result_msgs: List[Optional[ChatMessage]], content: str,
                              use_native_fc: bool) -> None:
        """
        处理工具执行结果，将结果消息追加到 messages 列表。
        """
        if use_native_fc:
            all_tool_calls = [
    ...
    def _handle_loop_timeout(self, state) -> None:
        """处理工具循环达到最大轮数仍未完成的情况"""
        if state.current_observation:
    ...
    # 工具执行辅助：安全检查方法
    # ============================================================
    def _handle_tool_not_found(self, tool_name: str, tc_id: str, use_native_fc: bool) -> ChatMessage:
        """处理工具不存在的情况，提供模糊匹配推荐。"""
        ...
        if use_native_fc:
            return ChatMessage(role="tool", content=error_feedback, tool_call_id=tc_id, name=tool_name)
        return ChatMessage("user", f"工具 '{tool_name}' 不存在。可用工具: {', '.join(self.tools.keys())}。请用其他工具或直接回答。")

    def _is_pattern_detection_enabled(self) -> bool:
        """检查 P3.3 模式检测是否启用。"""
        try:
            _raw = self.config.raw() if hasattr(self.config, 'raw') else {}
            return _raw.get("security", {}).get("pattern_detection", {}).get("enabled", True)
        except Exception:
            logger.debug(f"静默异常 [castorice/agent/tool_loop.py:314]")
            return True

    def _check_pattern_detection(self, tool_name: str, tool_args: Any, tc_id: str,
                                  use_native_fc: bool, state) -> Optional[ChatMessage]:
        """
        P3.3 模式检测：检查操作是否触发风险模式。
        返回 ChatMessage 表示被阻断，None 表示通过。
        """
        try:
            ...
            check_result = detector.check_operation(tool_name, tool_args)

            if check_result.get("critical"):
                blocked_names = [a.get("pattern_name", "") for a in check_result["critical"]]
                blocked_descs = "; ".join(
                    f"[{a['severity'].upper()}] {a['pattern_name']}: {a['description']}"
                    for a in check_result["critical"]
                )
                block_reason = f"[安全阻断 - P3.3 模式检测] 当前操作序列触发了以下严重风险模式: {blocked_descs}"
                logger.warning(f"P3.3 CRITICAL 阻断工具执行: {tool_name} | 触发的模式: {blocked_names}")
                with _state_tool_calls_lock:
                    state.tool_calls.append({
                        "tool_name": tool_name,
                        "arguments": tool_args,
                        "result": block_reason,
                        "pattern_alerts": check_result["alerts"],
                        "timestamp": datetime.now(timezone.utc).isoformat(),
                    })
                audit = getattr(self, 'audit_logger', None) or _get_audit_logger()
                if audit:
                    audit.log_tool_call(
                        user_id=state.session_id, session_id=state.session_id,
                        tool_name=tool_name, args=tool_args,
                        result=block_reason[:200], risk_level="critical",
                    )
                if use_native_fc:
                    return ChatMessage(role="tool", content=block_reason, tool_call_id=tc_id, name=tool_name)
                return ChatMessage("user", block_reason + " 请直接用你自己的话回答用户。")

            if check_result.get("high"):
                high_names = [a.get("pattern_name", "") for a in check_result["high"]]
                logger.warning(f"P3.3 HIGH 级别模式告警 (tool={tool_name}): {high_names}")
                state.pattern_alerts = getattr(state, 'pattern_alerts', [])
                state.pattern_alerts.extend(check_result["high"])
        except Exception as e:
            logger.warning(f"P3.3 模式检测失败: {e}")
        return None

    def _is_authorization_enabled(self) -> bool:
        """检查渐进授权系统是否启用。"""
        try:
            _raw = self.config.raw() if hasattr(self.config, 'raw') else {}
            return _raw.get("security", {}).get("authorization", {}).get("enabled", True)
        except Exception:
            logger.debug(f"静默异常 [castorice/agent/tool_loop.py:372]")
            return True

    def _check_authorization(self, tool_name: str, tc_id: str,
                              use_native_fc: bool) -> Optional[ChatMessage]:
        """
        P3.1 渐进授权检查：检查工具是否有权限执行。
        返回 ChatMessage 表示被拒绝，None 表示通过。
        """
        try:
            ...
            if not allowed:
                ...
                if use_native_fc:
                    return ChatMessage(role="tool", content=refuse_reason, tool_call_id=tc_id, name=tool_name)
    ...
    def _execute_single_tool_call(
        ...
        # 1. 工具不存在 → 模糊匹配推荐
        if tool_name not in self.tools:
            return self._handle_tool_not_found(tool_name, tc_id, use_native_fc)
        ...
        # 3. P3.3 主动模式检测 + 4. 渐进授权检查
        pd_enabled = self._is_pattern_detection_enabled()
        if pd_enabled:
            blocked = self._check_pattern_detection(tool_name, tool_args, tc_id, use_native_fc, state)
            if blocked is not None:
                return blocked
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"授权结果记录失败(成功): {e}")

            if pd_enabled:
                try:
                    from castorice.security.pattern_detector import get_pattern_detector
                    _pd = get_pattern_detector()
                    _cat = _TOOL_CATEGORY_MAP.get(tool_name, "other")
                    _pd.record(_cat, f"[result] {tool_name}: {result_str[:200]}")
                except Exception as e:
                    logger.warning(f"P3.3 执行后模式检测记录失败: {e}")
