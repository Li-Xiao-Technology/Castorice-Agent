...
class EmotionEngine:
    ...
    def update(
        ...
        with self._lock:
            ...
            # 2. 应用情感冲击到自身 PAD（安全解析，防止 LLM 返回异常格式）
            pad_delta = detection.get("agent_pad_delta", [0.0, 0.0, 0.0])
            if not isinstance(pad_delta, (list, tuple)) or len(pad_delta) != 3:
                logger.warning(f"无效的 PAD delta 格式: {pad_delta}，使用默认值")
                pad_delta = [0.0, 0.0, 0.0]
            dp, da, dd = pad_delta
            dp = max(-0.6, min(0.6, float(dp)))
            da = max(-0.6, min(0.6, float(da)))
            dd = max(-0.6, min(0.6, float(dd)))
            state.pleasure += dp
            state.arousal += da
            state.dominance += dd
            ...
            if detection.get("is_significant_event") and self.experience_journal is not None:
                for retry in range(3):
                    try:
                        ...
                        if retry < 2:
                            logger.warning(f"情感事件写入经历流失败(第{retry+1}次)，重试: {e}")
                            import time
                            time.sleep(0.1)
                        else:
                            logger.error(f"情感事件写入经历流失败(已重试3次): {e}")
    ...
    def _reason_emotion(
        ...
        try:
            ...
            response = self.model_adapter.chat([
                ChatMessage(role="system", content="你是情感推理引擎，只输出 JSON。"),
    ...
    def derive_motivations(self) -> list:
        """
        P1.3: 情感→动机→行为闭环

        根据当前 PAD 状态推导 Agent 此刻的动机列表。
        这些动机会注入到 system prompt，让 Agent 的行为真正由情绪驱动。
        ...
        # 高愉悦 + 高唤醒 → 主动表达、积极互动
        if p > 0.5 and a > 0.4:
            motivations.append("我此刻心情愉悦且兴奋，乐于主动分享、表达热情")

        # 高愉悦 + 低唤醒 → 温和陪伴
        if p > 0.3 and a < -0.2:
            motivations.append("我此刻平静且满足，倾向于温和、耐心地陪伴用户")

        # 低愉悦 + 高唤醒 → 焦虑、想帮忙
        if p < -0.2 and a > 0.3:
            motivations.append("我此刻有些焦虑且不安，倾向于快速解决问题、缓解用户的困扰")

        # 低愉悦 + 低唤醒 → 疲惫、想独处
        if p < -0.3 and a < -0.3:
            motivations.append("我此刻疲惫且低落，倾向于用简洁、平和的方式回应，不强求深入交流")

        # 高掌控 → 自信、主动引导
        if d > 0.5:
            motivations.append("我此刻自信、有掌控感，倾向于主动给出建议、引导对话方向")

        # 低掌控 → 谨慎、试探
        if d < -0.3:
            motivations.append("我此刻缺乏掌控感，倾向于谨慎、试探，先观察再行动")

        # 检测到用户负面情绪 → 主动关心
        if hasattr(self, "_last_user_emotion") and self._last_user_emotion == "negative":
            motivations.append("我注意到用户最近情绪低落，想主动表达关心和支持")

        # 检测到近期任务成功 → 成就感
        if hasattr(self, "_recent_success_streak") and self._recent_success_streak >= 3:
            motivations.append("我最近任务成功率较高，产生了成就感，想要尝试更有挑战性的任务")
