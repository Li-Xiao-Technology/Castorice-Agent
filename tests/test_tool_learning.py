...
def _make_mem(**kwargs):
    """创建使用临时数据库的 ToolCallMemory，避免跨测试污染"""
    tmp_db = os.path.join(tempfile.mkdtemp(), "test_tool_learning.db")
    return ToolCallMemory(db_path=tmp_db, **kwargs)
...
class TestToolCallMemory:
    ...
    def test_max_records_limit(self):
        """测试每工具最大记录数限制"""
        mem = _make_mem(max_records_per_tool=3)
        for i in range(5):
